...
soup=BeautifulSoup(open("instagraminfluencers.html"),"lxml")

outfile=open('top_500_instagram_influencers.json',"a")
for row in soup.findAll("div",attrs={"style":"width: 860px; background: #f8f8f8;; padding: 0px 20px; color:#444; font-size: 10pt; border-bottom: 1px solid #eee; line-height: 30px;"}):
    ...
    details['name'] = item[2].text
    details['posts/media'] = item[3].text
    details['followers'] = item[4].text
    details['following'] = item[5].text
